    ...
    FactionLogoCacheService factionLogoCacheService) : IImportService
{
    ...
    public async IAsyncEnumerable<SuccessWithStringResult> ImportMetadataAsync()
    {
        ...
        foreach (var factionId in factionIds)
        {
            ...
            if (factionLogoCacheService is not null && latestArmy.Resume is not null)
            {
    ...
    public async IAsyncEnumerable<SuccessWithStringResult> ImportAllDataAsync()
    {
        ...
        if (factionLogoCacheService is not null)
        {
            ...
            var logoCacheResult = await factionLogoCacheService.CacheAllAsync(metadata.Factions);
            var debugFaction = metadata.Factions.FirstOrDefault(x => x.Id == FactionLogoCacheService.DebugFactionId);
            var debugInfo = factionLogoCacheService.GetDebugInfo(FactionLogoCacheService.DebugFactionId, debugFaction?.Logo);
