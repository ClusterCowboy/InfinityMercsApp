...
public sealed class FactionProvider(ISQLiteRepository sqliteRepository) : IFactionProvider
{
    ...
    {
        return sqliteRepository.GetAll<Unit>(x => x.FactionId == factionId && (x.Slug == null || !x.Slug.StartsWith(MercSlugPrefix)), null);
    }
    ...
    public Unit? GetUnit(int factionId, int unitId)
    {
        return sqliteRepository
            .GetAll<Unit>(x => x.FactionId == factionId
                               && x.UnitId == unitId
                               && (x.Slug == null || !x.Slug.StartsWith(MercSlugPrefix)), null)
            .FirstOrDefault();
    }
    ...
    public IReadOnlyList<Unit> SearchUnits(string searchTerm, int? factionId = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(term))
        {
            ...
            return sqliteRepository.GetAll<Unit>(x => x.Slug == null || !x.Slug.StartsWith(MercSlugPrefix)).Take(250).ToList();
        }

        if (factionId.HasValue)
        {
            return sqliteRepository.GetAll<Unit>(x => x.Name.Contains(term) && x.FactionId == factionId.Value && (x.Slug == null || !x.Slug.StartsWith(MercSlugPrefix)));
        }

        return sqliteRepository.GetAll<Unit>(x => x.Name.Contains(term) && (x.Slug == null || !x.Slug.StartsWith(MercSlugPrefix)));
    }

    /// <inheritdoc/>
    public IReadOnlyList<Resume> GetResumeByFaction(int factionId)
    {
        return sqliteRepository.GetAll<Resume>(x => x.FactionId == factionId && (x.Slug == null || !x.Slug.StartsWith(MercSlugPrefix)));
    }
